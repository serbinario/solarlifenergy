    ...
    public function grid(Request $request)
    {
        ...
        return Datatables::of($rows)
            ...
            ->addColumn('action', function ($row) {
                ...
                $acao = '<form id="' . $row->id   . '" method="POST" action="solicitacaoEntrega/' . $row->id   . '/destroy" accept-charset="UTF-8">
                            <input name="_method" value="DELETE" type="hidden">
                            <input name="_token" value="'.$this->token .'" type="hidden">
                            <div class="btn-group btn-group-xs pull-right" role="group">';

                $user =  Auth::user();
                if($user->hasPermissionTo('update.log.entrega')) {
                    $acao .= '<a href="/report/' . $row->id . '/solicitacaoEntrega" class="btn btn-primary" target="_blank" title="Solicitação de entrega">
                                    <span class="glyphicon glyphicon-file" aria-hidden="true"></span>
                                </a> ';
                    $acao .= '<a href="solicitacaoEntrega/' . $row->id . '/edit" class="btn btn-primary" title="Edit">
