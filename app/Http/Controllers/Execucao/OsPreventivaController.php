...
class OsPreventivaController extends Controller
{
    use UtilFiles;
    private $token;
    ...
    public function index()
    {
        ...
        return view('execucao.os_preventiva.index', compact('status'));
    }
        ...
        return Datatables::of($rows)
            ...
            ->addColumn('action', function ($row) use ($user) {
                ...
                if($user->hasRole('super-admin')) {
                    $acao =  '<form id="' . $row->id   . '" method="POST" action="execucao/os_corretiva/' . $row->id   . '/destroy" accept-charset="UTF-8">
                            <input name="_method" value="DELETE" type="hidden">
                            <input name="_token" value="'.$this->token .'" type="hidden">
                            <div class="btn-group btn-group-xs pull-right" role="group">';

                    $acao .= '<a href="osCorretiva/'.$row->id.'/edit" class="btn btn-primary" title="Edit">
    ...
    public function edit($id)
    {
        ...
        $status = StatusVisita::pluck('descricao','id')->all();
        //dd($ordemServico);
        return view('execucao.os_preventiva.edit', compact( 'ordemServico', 'users', 'status'));
    ...
    public function update($id, OsCorretivaFormRequest $request)
    {
        try {
            ...
            $os->update($data);
            return redirect()->route('osPreventiva.edit', $os->id)
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('osPreventiva.index')
